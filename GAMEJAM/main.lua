...
bgImage, bgWidth, bgScroll, bgSpeed,
currentMap = 1
bgImages = {}
totalScrollPx = 0
BG_REPEAT = 4
...
backTargetX, backTargetY = nil, nil
backRemoveIndex = nil

sounds = {}
countdownImgs = {}

local creditImg1, creditImg2
creditsTimer = 0
...
local rastroFadeOutPerSec = 0.35   
...
local obstaculoEscala = {
  ...
  tenderobosque  = { w = 2.2, h = 2.0 }, 
  caja           = { w = 1.0, h = 1.0 },
  ositoobstaculo = { w = 2.5, h = 2.5 },
  laguitoobstaculo = { w = 2.5, h = 2.0 },
  tibutron        = { w = 2.5, h = 1.5 },
  tenderomar      = { w = 2.2, h = 2.0 },
  perritomar      = { w = 1.3, h = 1.3 },
}
local obstaculoMovimiento = {
  ...
  tenderobosque          = nil,
  caja           = nil,
  tibutron        = { kind = "x", speed = 120, amplitude = 260, prob = 0.8 },
  perritomar      = { kind = "y", speed =  90, amplitude = 180, prob = 0.7 },
}
local obstaculosPorZona = {
  ciudad = { "cochecitolere", "puestoperritos", "perritomalo" },
  bosque = { "tenderobosque", "ositoobstaculo", "laguitoobstaculo" },
  oceano = { "tibutron", "tenderomar", "perritomar" },
}
...
local endSlideDuration = 0.8   
local endFadeDuration  = 0.3   
...
function seqIndex(n)
 local ciclo = BG_REPEAT * 3         
 local i     = n % ciclo
 if i < BG_REPEAT then
    return 1                           
  elseif i < BG_REPEAT * 2 then
    return 2                           
  else
    return 3                          
  end
...
function zonaDeSpawn()
  ...
  local group  = math.floor(base / BG_REPEAT)
  local idx    = (group % 3) + 1
  return zonaDeIndice(idx)
end
...
function love.load()
  ...
  sounds.music = love.audio.newSource("audio/fondo/chase.mp3", "stream")
  sounds.music:setLooping(true)
  
  sounds.menu = love.audio.newSource("audio/fondo/menu.mp3", "stream")
  ...
  love.graphics.newImage("sprites/fondos/fondomar.png"),}
  bgScroll = 0
  bgSpeed  = 500
  recalcBgScale()
  for i=1,#bgImages do
  ...
  endSlides[1] = love.graphics.newImage("sprites/fondos/finpartida/fin1.png")
  endSlides[2] = love.graphics.newImage("sprites/fondos/finpartida/fin2.png")
  endSlides[3] = love.graphics.newImage("sprites/fondos/finpartida/fin3.png")
  endSlides[4] = love.graphics.newImage("sprites/fondos/finpartida/fin4.png")
  for i=1,#endSlides do endSlides[i]:setFilter("linear","linear") end

  

  victoryBg = love.graphics.newImage("ui/win.png")
  sounds.victoria = love.audio.newSource("audio/fondo/victoria.mp3", "stream")
  sounds.victoria:setLooping(true)
  
  creditsImg = love.graphics.newImage("ui/botones/credits.png")
  
   local winScale   = buttonScale
  btnCredits = {
    scale = winScale,
    w     = creditsImg:getWidth()  * winScale,
    h     = creditsImg:getHeight() * winScale,
  }
  local winSpacing = 10
  local totalWinH = btnRestart.h + winSpacing + btnGOExit.h + winSpacing + btnCredits.h
  btnRestart.winY   = (h0 - totalWinH) / 2
  btnGOExit.winY    = btnRestart.winY + btnRestart.h + winSpacing
  btnCredits.y      = btnGOExit.winY  + btnGOExit.h  + winSpacing
  btnRestart.winX   = btnRestart.x
  btnGOExit.winX    = btnGOExit.x
  btnCredits.x      = (w0 - btnCredits.w) / 2
  ...
  headSprites[4] = love.graphics.newImage("sprites/pancho/pabajo1.png")
  headSprites[5] = love.graphics.newImage("sprites/pancho/pabajo2.png")
...
obstaculosSprites.tenderobosque = love.graphics.newImage("sprites/tenderobosque.png")
...
obstaculosSprites.tibutron      = love.graphics.newImage("sprites/tibutron.png")
obstaculosSprites.tenderomar    = love.graphics.newImage("sprites/tenderomar.png")
obstaculosSprites.perritomar    = love.graphics.newImage("sprites/perritomar.png")

countdownImgs = {
  love.graphics.newImage("ui/cuentaatras/tres.png"),
  love.graphics.newImage("ui/cuentaatras/dos.png"),
  love.graphics.newImage("ui/cuentaatras/uno.png"),
}

  creditImg1 = love.graphics.newImage("ui/creditos1.png")
  creditImg2 = love.graphics.newImage("ui/creditos2.png")
  ...
  exitImgWin = love.graphics.newImage("ui/botones/exitwin.png")
...
function love.update(dt)
  ...
  if gameState=="credits" then
    ...
    if sounds.music and sounds.music:isPlaying() then
      sounds.music:stop()
    end
    if sounds.menu and not sounds.menu:isPlaying() then
      sounds.menu:play()
    end

  else
    if sounds.menu and sounds.menu:isPlaying() then
      sounds.menu:stop()
    end
  end
...
if winPhase == "done" then
  ...
  endSlideIndex = 1
  endSlideTimer = 0
  if rastroWarningActive and sounds.perderrastro then
    sounds.perderrastro:stop()
  end
end
...
if gameState == "ending" then
  endSlideTimer = endSlideTimer + dt
  if endSlideTimer >= (endSlideDuration + endFadeDuration) then
    endSlideTimer = endSlideTimer - (endSlideDuration + endFadeDuration)
...
local camX = (gameState == "win" and frozenScrollX) or scrollX
 ...
 if gameState == "gameover" then
    ...
    -- Fondo de gameover
    love.graphics.draw(gameoverBg, 0, 0, 0,
      w0 / gameoverBg:getWidth(),
      h0 / gameoverBg:getHeight()
    )
    -- Botón Restart
    love.graphics.draw(restartImg,
      btnRestart.x, btnRestart.y,
      0, btnRestart.scale, btnRestart.scale
    )
    -- Botón Exit
    love.graphics.draw(exitImgWin or exitImg,
      btnGOExit.x, btnGOExit.y,
      0, btnGOExit.scale, btnGOExit.scale
    )
    return
  end
  ...
  if gameState=="credits" then
    love.graphics.setColor(1,1,1)
    if creditsTimer < 5 then
      love.graphics.draw(creditImg1, 0,0, 0, w0/creditImg1:getWidth(), h0/creditImg1:getHeight())
    else
      love.graphics.draw(creditImg2, 0,0, 0, w0/creditImg2:getWidth(), h0/creditImg2:getHeight())
    end
    return
  end
...
if gameState == "victory" then
  love.graphics.setColor(1,1,1)
  love.graphics.draw(victoryBg, 0, 0, 0,
    w0 / victoryBg:getWidth(),
    h0 / victoryBg:getHeight()
  )
  love.graphics.draw(restartImg, btnRestart.winX + 235, btnRestart.winY + 220, 0, btnRestart.scale, btnRestart.scale)
  love.graphics.draw(exitImgWin,  btnGOExit.winX,   btnGOExit.winY + 155,   0, btnGOExit.scale,   btnGOExit.scale)
  love.graphics.draw(creditsImg,   btnCredits.x + 300,      btnCredits.y + 150, 0, btnCredits.scale,  btnCredits.scale)
  return
end
...
local targetX = cabeza.x
local targetY = cabeza.y

local headDrawX = lerp(headPrevX, targetX, alpha) + camX
local headDrawY = lerp(headPrevY, targetY, alpha)


local sprite, rot
if headFacing == "s" then

  local idx = 4 + ((headAnimFrame - 1) % 2) 
  sprite = headSprites[idx]
  rot = 0
elseif headFacing == "w" then
...
if sprite then
  local scale = tamañoPerro / sprite:getWidth()
  local cx = headDrawX + tamañoPerro/2
  local cy = headDrawY + tamañoPerro/2
  love.graphics.setColor(1,1,1,1)
  love.graphics.draw(
    sprite,
    cx, cy,                  
...
if gameState == "playing" and offTrailGraceTimer >= offTrailGrace and offTrailTimer > 0 then
  local img
  if offTrailTimer <= 3 and offTrailTimer > 2 then
    img = countdownImgs[1]   
  elseif offTrailTimer <= 2 and offTrailTimer > 1 then
    img = countdownImgs[2]    
  elseif offTrailTimer <= 1 and offTrailTimer > 0 then
    img = countdownImgs[3]  
  end
  if img then
    local iw, ih = img:getDimensions()
    local scale = 0.25
    local x = w0 - iw * scale - 10 
    local y = 10
    love.graphics.setColor(1,1,1,1)
 ...
 if gameState == "fadeout" or gameState == "gameover" then
  ...
  local t = endSlideTimer
  local alphaCurr, alphaNext = 1, 0
  if t > endSlideDuration then
    local k = math.min(1, (t - endSlideDuration) / endFadeDuration)
    alphaCurr = 1 - k
    alphaNext = k
  end

  local function drawCentered(img, alpha)
    if not img or alpha <= 0 then return end
    local iw, ih = img:getWidth(), img:getHeight()
    local sx = math.max(w0 / iw, h0 / ih)
    local dx = (w0 - iw * sx) * 0.5
    local dy = (h0 - ih * sx) * 0.5
    love.graphics.setColor(1,1,1,alpha)
    love.graphics.draw(img, dx, dy, 0, sx, sx)
  end

  love.graphics.clear(1,1,1,1)
  drawCentered(curr, alphaCurr)
  drawCentered(next, alphaNext)
  love.graphics.setColor(1,1,1,1)
  return
end
...
if gameState == "victory" and button == 1 then

    local restartX, restartY = btnRestart.winX + 235, btnRestart.winY + 220
    local restartW = restartImg:getWidth() * btnRestart.scale
    local restartH = restartImg:getHeight() * btnRestart.scale

    local exitX, exitY = btnGOExit.winX, btnGOExit.winY + 155
    local exitW = exitImgWin:getWidth() * btnGOExit.scale
    local exitH = exitImgWin:getHeight() * btnGOExit.scale

    local creditsX, creditsY = btnCredits.x + 300, btnCredits.y + 150
    local creditsW = creditsImg:getWidth() * btnCredits.scale
    local creditsH = creditsImg:getHeight() * btnCredits.scale

    if x >= restartX and x <= restartX + restartW
    and y >= restartY and y <= restartY + restartH then
      sounds.victoria:stop()
      iniciarJuego()
      gameState = "playing"
      return
    end

    if x >= exitX and x <= exitX + exitW
    and y >= exitY and y <= exitY + exitH then
      love.event.quit()
      return
    end
    if x >= creditsX and x <= creditsX + creditsW
    and y >= creditsY and y <= creditsY + creditsH then
      creditsTimer = 0
      gameState = "credits"
      return
    end
...
function moverPerroBien(direccion)
  ...
  if nuevaX ~= cabeza.x or nuevaY ~= cabeza.y then
    ...
    if direccion == "w" or direccion == "s" or direccion == "a" then
    headFacing = direccion
  end
  end
end
